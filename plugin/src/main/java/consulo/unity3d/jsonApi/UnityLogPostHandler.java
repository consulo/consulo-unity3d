...
{
	private final UnityLogHandler myUnityLogHandler;

	@Inject
	public UnityLogPostHandler(Application application)
	{
		super("unityLog", UnityLogPostHandlerRequest.class);
		myUnityLogHandler = application.getMessageBus().syncPublisher(UnityLogHandler.class);
	}
	...
	{
		myUnityLogHandler.handle(request);
