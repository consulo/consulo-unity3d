...
import consulo.unity3d.base.editor.UnityEditorCommunicator;
...
import consulo.unity3d.base.UnityPluginValidator;
import consulo.unity3d.base.bundle.Unity3dBundleType;
import consulo.unity3d.base.bundle.Unity3dDefineByVersion;
import consulo.unity3d.base.editor.UnityRequestDefines;
...
import consulo.unity3d.base.module.Unity3dChildMutableModuleExtension;
...
import consulo.unity3d.base.packages.orderEntry.Unity3dPackageOrderEntryModel;
import consulo.unity3d.base.packages.orderEntry.Unity3dPackageOrderEntryType;
...
public class Unity3dProjectImporter {
    ...
    private static final String UNITY_NUNIT_EXTENSION_ID = "unity3d-nunit-child";
    public static final String ROOT_MODULE_SUFFIX = " (root)";
    ...
    public static void syncProjectStep(@Nonnull final Project project, @Nullable final Sdk sdk, @Nullable UnityOpenFilePostHandlerRequest requestor, final boolean runValidator) {
        ...
        Task.Backgroundable.queue(project, "Fetching defines from UnityEditor", indicator ->
        {
            ...
            if (!UnityEditorCommunicator.getInstance().request(project, request, true)) {
                token.finish(null);
    ...
    private static void importAfterDefines(@Nonnull final Project project,
        ...
        try {
            ...
            if (EarlyAccessProgramManager.is(UnityOldImportEapDescriptor.class)) {
            ...
            UnityRunConfigurationCreator.getInstance().createDefaultConfiguration(project);
        }
        finally {
        ...
        if (requestor != null) {
            UIUtil.invokeLaterIfNeeded(() -> UnityFileOpener.openFile(project, requestor));
        }
    }
    ...
    private static Module createPackageModule(@Nonnull VirtualFile packageDir, @Nonnull ModifiableModuleModel modifiableModuleModels, @Nonnull UnityProjectImportContext context) {
        ...
        fillModuleDependencies(module, modifiableModel, List.of(packageDir, context.getProject().getBaseDir().findChild(Unity3dPaths.ASSETS_DIRECTORY)), it ->
        {
    ...
    private static Module createAssemblyCSharpModule(ModifiableModuleModel newModel,
        ...
        VirtualFile mainDir = context.getProject().getBaseDir().findFileByRelativePath(Unity3dPaths.ASSETS_DIRECTORY);
    ...
    private static Module createAssemblyCSharpModuleFirstPass(ModifiableModuleModel newModel, MultiMap<Module, VirtualFile> virtualFilesByModule, UnityProjectImportContext context) {
        ...
        for (String passPath : Unity3dPaths.FIRST_PASS_PATHS) {
    ...
    private static Module createAssemblyCSharpModuleEditor(ModifiableModuleModel newModel, MultiMap<Module, VirtualFile> virtualFilesByModule, UnityProjectImportContext context) {
        ...
        final VirtualFile assetsDir = baseDir.findFileByRelativePath(Unity3dPaths.ASSETS_DIRECTORY);
        if (assetsDir != null) {
        ...
        return createAndSetupModule("Assembly-CSharp-Editor", newModel, moduleDirs, layer ->
        {
            ...
            if (isVersionHigherOrEqual(unityBundle, "5.3.0")) {
                ...
                // enable nunit - by id, so this module does not depend on the nunit extension class
                MutableModuleExtension<?> nunitExtension = layer.getExtensionWithoutCheck(UNITY_NUNIT_EXTENSION_ID);
                if (nunitExtension != null) {
                    nunitExtension.setEnabled(true);
                }
            }
    ...
    private static Version parseBundleVersion(@Nullable Sdk unityBundle) {
        ...
        return Unity3dPaths.parseVersion(currentVersionString);
    }
    ...
    private static boolean isVersionHigherOrEqual(@Nullable Sdk unityBundle, @Nonnull String requiredVersionString) {
        ...
        Version requiredVersion = Unity3dPaths.parseVersion(requiredVersionString);
    ...
    /**
     * Name of the module holding the unity root extension and the sdk.
     * <p>
     * Suffixed so that it cannot collide with an assembly definition named after the project - such an asmdef
     * used to take over the root module, leaving the project without an sdk.
     */
    @Nonnull
    public static String getRootModuleName(@Nonnull Project project) {
        return project.getName() + ROOT_MODULE_SUFFIX;
    }
    ...
    public static Module createRootModule(@Nonnull Project project,
        ...
        if (rootModuleExtension != null) {
            ...
            rootModule = newModel.newModule(getRootModuleName(project), project.getBasePath());
        }
